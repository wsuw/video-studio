    ...
    try:
        ...
        if spk_path.startswith("http://") or spk_path.startswith("https://"):
            ...
            if not os.path.exists(spk_path):
                ...
                if not os.path.exists(spk_path):
                    ...
                    index_tts_spk = os.path.join("D:\\AntigravityProject\\index-tts", spk_audio_prompt)
                    if os.path.exists(index_tts_spk):
                        spk_path = index_tts_spk
                    else:
                        raise FileNotFoundError(f"Speaker reference audio file not found: {spk_audio_prompt}")
        ...
        if emo_path:
            if emo_path.startswith("http://") or emo_path.startswith("https://"):
                ...
                if not os.path.exists(emo_path):
                    ...
                    if not os.path.exists(emo_path):
                        index_tts_emo = os.path.join("D:\\AntigravityProject\\index-tts", emo_audio_prompt)
                        if os.path.exists(index_tts_emo):
                            emo_path = index_tts_emo
                        else:
                            raise FileNotFoundError(f"Emotion reference audio file not found: {emo_audio_prompt}")

        # 3. Call the IndexTTS2 api.py service (port 8000)
        index_tts_url = os.getenv("INDEX_TTS_API_URL", "http://127.0.0.1:8000/synthesize")
        ...
        if response.status_code != 200:
            ...
            try:
                ...
                pass
            raise Exception(f"IndexTTS2 API synthesis failed ({response.status_code}): {error_detail}")
        ...
        print(f"[{time.strftime('%H:%M:%S')}] TTS successfully generated in {elapsed:.2f}s! S3 URL: {web_url}")
